<template>
    <Form ref="formRef" :resolver="resolver" :initialValues="initialValues" @submit="onSubmit" class="flex flex-col gap-4 w-fit min-w-[200px]">
        <FormField v-slot="$field" name="id" class="flex flex-col gap-1" hidden>
            <label for="id" class="fw-bold">id</label>
            <InputText v-bind="$field.props" id="id" placeholder="id" :disabled="isSubmitting" />
        ...
        <FormField v-slot="$field" name="email" class="flex flex-col gap-1">
            ...
            <InputText v-bind="$field.props" id="email" :placeholder="t('user.email_placeholder')" :disabled="isSubmitting" />
            <Message v-if="$field.invalid" severity="error" size="small" variant="simple">
                {{ $field.error?.message }}
            </Message>
        </FormField>
        <div class="flex flex-row">
            <FormField v-slot="$field" name="first_name" class="flex flex-col gap-1 mr-3 w-auto">
                ...
                <InputText v-bind="$field.props" id="first_name" :placeholder="t('user.first_name_placeholder')" :disabled="isSubmitting" />
                <Message v-if="$field.invalid" severity="error" size="small" variant="simple">
                    {{ $field.error?.message }}
                </Message>
            </FormField>
            <FormField v-slot="$field" name="last_name" class="flex flex-col gap-1 w-auto">
                ...
                <InputText v-bind="$field.props" id="last_name" :placeholder="t('user.last_name_placeholder')" :disabled="isSubmitting" />
                <Message v-if="$field.invalid" severity="error" size="small" variant="simple">
                    {{ $field.error?.message }}
                </Message>
            </FormField>
        ...
        <FormField v-slot="$field" name="groups" class="flex flex-col gap-1">
            <label for="groups" class="fw-bold">{{ t('user.groups') }}</label>
            <MultiSelect :options="groups" optionLabel="label" optionValue="value" v-bind="$field.props" id="groups" :placeholder="t('user.groups_placeholder')" filter :disabled="isSubmitting" />
            <Message v-if="$field.invalid" severity="error" size="small" variant="simple">
                {{ $field.error?.message }}
            </Message>
        </FormField>
        <Button type="submit" severity="secondary" :label="t('form.save')" :loading="isSubmitting" :disabled="isSubmitting" />
    </Form>
</template>
...
const onSubmit = async (event: FormSubmitEvent<Record<string, any>>) => {
    if (isSubmitting.value || !event.values) return;
    clearApiErrors();
    isSubmitting.value = true;

    try {
        const { data } = await api.post('/construction/manager/user/', event.values);
