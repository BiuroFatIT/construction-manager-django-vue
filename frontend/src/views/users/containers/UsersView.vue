...
const configuration: Config[] = [
      ...
      return (
        <div style="display: flex; flex-wrap: wrap; gap: 6px;">
          {row.groups.map((group: string) => (
            <Tag
              severity={group === 'Owners' ? 'warning' : group === 'Administrators' ? 'success' : 'info'}
              value={group}
            />
          ))}
        </div>
  ...
  { field: 'last_login', header: 'Ostatnie Logowanie', sortable: true, filterable: true, filterType: 'datetime' },
  { field: 'date_joined', header: 'Data Utworzenia', sortable: true, filterable: true, filterType: 'datetime' },
];
</script>
